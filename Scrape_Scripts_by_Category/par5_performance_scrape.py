...
docTitle = 'par5_performance'
url = 'https://www.pgatour.com/stats/stat.173.y'
    ...
    if statTable:
        ...
        for tr in statTable.find_all('tr'):
            ...
            if count_tr > 0:
                ...
                par5_avg = stat_record[5]
                #print(total_sg_app_green)
                total_par = stat_record[6]
                #print(measured_rounds)
                stat_data.append([year_id,rank,player_name,rounds,to_par,par5_avg,total_par])
...
df_sd = pd.DataFrame(stat_data, columns = ['year','rank','player_name','rounds','to_par','par5_avg','total_par'])
